...
public class LoginAndAuthHelper implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, ResultCallback<People.LoadPeopleResult> {
    ...
    private Activity getActivity(String methodName) {
        ...
        if (activity == null) {
            Log.d(TAG, "Helper lost Activity reference, ignoring (" + methodName + ")");
        }
        return activity;
    }

    public void retryAuthByUserRequest() {
        Log.d(TAG, "Retrying sign-in/auth (user-initiated).");
        if (!mGoogleApiClient.isConnected()) {
            ...
            Log.d(TAG, "No need to retry auth: GoogleApiClient is connected and we have auth token.");
        }
    }
    ...
    public void onConnected(Bundle bundle) {
        ...
        Person cp = Plus.PeopleApi.getCurrentPerson(mGoogleApiClient);
    ...
    public void onResult(People.LoadPeopleResult loadPeopleResult) {
        ...
        if (loadPeopleResult.getStatus().isSuccess()) {
            ...
            int status = loadPeopleResult.getStatus().getStatusCode();
            if (status == 7){

            }
            Log.e(TAG, "Failed to load plus profile, error " + loadPeopleResult.getStatus().getStatusCode());
